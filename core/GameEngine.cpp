...
// GameEngine.cpp
//
// ALL function implementations are enclosed within namespace Engine { ... }.
// No implementation code exists outside the namespace block.
//
// updateGameplaySimulation uses:
//   - #ifdef __DREAMCAST__ : console-specific clock/sync logic
//   - #else                : Emscripten/Web EM_ASM_DOUBLE audio-time logic
// =============================================================================
...
void GameEngine::tick() {
    ...
    switch (m_currentState) {
        ...
        case EngineState::SongSelect:
            if (pressed & LANE_MASKS[0] && m_selectedSong > 0)
                --m_selectedSong;
            if (pressed & LANE_MASKS[1] && m_selectedSong < (TOTAL_SONGS - 1))
                ++m_selectedSong;
            if (pressed & static_cast<PAL::InputState>(PAL::InputAction::Confirm)) {
                loadChart(m_selectedSong == CD_STREAM_TRACK_ID
                          ? nullptr
...
void GameEngine::updateGameplaySimulation(PAL::InputState pressed) {
#ifdef __DREAMCAST__
    // --- Dreamcast: sync to audio hardware every 4 ticks, predict in between ---
    m_syncTickCounter++;
    if ((m_syncTickCounter & 0x03) == 0) {
        m_localTrackAccumulator =
            static_cast<PAL::FP16>(s_audio->getTrackProgress());
    } else {
        uint32_t predicted =
            static_cast<uint32_t>(m_localTrackAccumulator) + 0x0440u;
        m_localTrackAccumulator =
            (predicted > 0xFFFFu) ? 0xFFFF
                                  : static_cast<PAL::FP16>(predicted);
    }
#else
    ...
    m_combo     = (fr.missCount + fr.holdDropCount > 0)
                  ? 0
                  : m_combo + fr.perfectCount;
    ...
    auto processNotes = [&](uint16_t head, uint16_t tail, bool isStreaming) {
        ...
        for (uint16_t i = head; i != limit; ++i) {
            ...
            if (m_shipLane == note.lane) {
                ...
                if (pushBlockToGrid(note.lane, note.flags))
...
bool GameEngine::pushBlockToGrid(uint8_t lane, uint8_t blockType) {
    ...
    m_puzzleGrid[lane] |=
        (static_cast<uint16_t>(blockType & 0x07) << shiftAmount);
    m_gridHeights[lane]++;
...
void GameEngine::checkAndResolveMatches() {
    ...
    for (uint8_t l = 0; l < 3; ++l) {
        ...
        for (uint8_t r = 0; r < m_gridHeights[l]; ++r)
            unpackedGrid[l][r] = (laneData >> (r * 3)) & 0x07;
    }

    // Vertical runs of 3 within each lane
    for (uint8_t l = 0; l < 3; ++l) {
        ...
        for (uint8_t r = 0; r <= m_gridHeights[l] - 3; ++r) {
            ...
            if (unpackedGrid[l][r + 1] == color &&
                unpackedGrid[l][r + 2] == color) {
                markedMasks[l] |= (7 << r);
                foundMatch = true;
            }
        }
    ...
    // Horizontal runs of 3 across all lanes at the same row
    for (uint8_t r = 0; r < GRID_MAX_ROWS; ++r) {
        if (m_gridHeights[0] <= r ||
            m_gridHeights[1] <= r ||
            m_gridHeights[2] <= r) continue;
    ...
    m_score += static_cast<uint16_t>(
        totalCleared * 500u * (m_combo > 0 ? (m_combo >> 2) + 1 : 1));
...
void GameEngine::renderGameplayScene() const {
    ...
    int32_t diff = static_cast<int32_t>(m_localTrackAccumulator)
                 - static_cast<int32_t>(m_cameraZ);
    m_cameraZ += static_cast<PAL::FP16>((diff * 0x0C00) >> 16);

    const uint32_t viewLookAhead =
        (static_cast<uint32_t>(m_cameraZ) + 0x2400u) > 0xFFFFu
        ? 0xFFFFu
        : (m_cameraZ + 0x2400u);

    s_graphics->drawVoxelHighway(320 << 8, 200 << 8, 0,
                                 static_cast<PAL::SFP16>(energy << 7),
                                 m_selectedSong);
    ...
    auto renderNotes = [&](uint16_t scanIdx, uint16_t limit, bool streaming) {
        while (scanIdx != limit) {
            ...
            if (!alreadyHit) {
                int32_t zDist =
                    static_cast<int32_t>(n.timeline) -
                    static_cast<int32_t>(m_cameraZ);
                if (zDist > 0) {
                    PAL::SFP16 s =
                        static_cast<PAL::SFP16>((0x100000 / (zDist + 1)) >> 8);
                    s_graphics->drawBlock3D(
                        static_cast<PAL::SFP16>(
                            320 + (((LANE_X[n.lane] - 320) * s) >> 8)),
                        static_cast<PAL::SFP16>(
                            200 + (((LANE_HIT_Y - 200) * s) >> 8) +
                            (energy << 2)),
                        s, n.flags);
                }
            }
        ...
        for (uint8_t r = 0; r < m_gridHeights[l]; ++r) {
            ...
            if (t != 0)
                s_graphics->drawBlock(
                    static_cast<PAL::SFP16>((500 + (l * GRID_BLOCK_SPACING)) << 8),
                    static_cast<PAL::SFP16>(GRID_BASE_Y - (r * GRID_BLOCK_SPACING)),
                    1 << 8, t);
        }
    }
...
void GameEngine::renderTitleScreen() const {
    s_graphics->drawVoxelColumnGlow(
        static_cast<PAL::SFP16>(120 << 8),
        static_cast<PAL::SFP16>(520 << 8),
        static_cast<PAL::SFP16>(200 << 8),
        static_cast<PAL::SFP16>(280 << 8),
        static_cast<PAL::SFP16>(  5 << 8),
        2, 255);
...
void GameEngine::renderResultsScreen() const {
    s_graphics->drawBlock(
        ...
        static_cast<PAL::SFP16>(240 << 8),
        static_cast<PAL::SFP16>( 15 << 8),
        7);
    s_graphics->drawHUD(m_score, m_combo, m_missCount, 0);
}

} // namespace Engine
// ^ Everything above this line is inside namespace Engine.
//   Nothing follows — no stray implementations in the global namespace.
